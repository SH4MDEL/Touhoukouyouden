...
void ProcessPacket(int ci, unsigned char packet[])
{
	switch (packet[2]) {
	case SC_MOVE_OBJECT: {
		SC_MOVE_OBJECT_PACKET* objectPacket = reinterpret_cast<SC_MOVE_OBJECT_PACKET*>(packet);
		if (objectPacket->id < MAX_CLIENTS) {
			...
			if (ci == id) {
				if (0 != objectPacket->move_time) {
					auto now = static_cast<unsigned int>(duration_cast<milliseconds>(high_resolution_clock::now().time_since_epoch()).count());
					auto prev = objectPacket->move_time;
					if (now >= prev) {
						auto d_ms = now - prev;
	...
	case SC_ADD_OBJECT: {
		SC_ADD_OBJECT_PACKET* objectPacket = reinterpret_cast<SC_ADD_OBJECT_PACKET*>(packet);
		if (objectPacket->id < MAX_CLIENTS) {
			int id = client_map[objectPacket->id];
			if (-1 != id) {
				g_clients[id].x = objectPacket->coord.x;
	...
	case SC_LOGIN_INFO:
	{
		...
		active_clients++;
		SC_LOGIN_INFO_PACKET* loginPacket = reinterpret_cast<SC_LOGIN_INFO_PACKET*>(packet);
		int my_id = ci;
